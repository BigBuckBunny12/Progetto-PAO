...
// Visitor pattern
void MediaCreationFormVisitor::visit(const Book& book) {
    addLineEdit("Titolo", "title", book.getTitle(), false);
    addLineEdit("Anno di pubblicazione", "year", QString::number(book.getPublicationYear()), true);
    addLineEdit("Autore", "author", book.getAuthor(), false);
    addSpinBox("Numero pagine", "pages", book.getTotalPages());
    addLineEdit("Editore", "publisher", book.getPublisher(), false);
    addComboBox<Book::Genre>("Genere", "genre", Book::genreLabels(), book.getGenre());
    addImageSelector("Copertina", "cover", book.getCoverImagePath());
}

void MediaCreationFormVisitor::visit(const Movie& movie) {
    addLineEdit("Titolo", "title", movie.getTitle(), false);
    addLineEdit("Anno di pubblicazione", "year", QString::number(movie.getPublicationYear()), true);
    addTimeEdit("Durata", "duration", movie.getDuration());
    addLineEdit("Regista", "producer", movie.getProducer(), false);
    addImageSelector("Copertina", "cover", movie.getCoverImagePath());
}

void MediaCreationFormVisitor::visit(const Article& article) {
    addLineEdit("Titolo", "title", article.getTitle(), false);
    addLineEdit("Anno di pubblicazione", "year", QString::number(article.getPublicationYear()), true);
    addLineEdit("Fonte", "source", article.getSource(), false);
    addLineEdit("DOI", "doi", article.getDoi(), false);
    addLineEdit("Numero fascicolo", "issueNumber", QString::number(article.getIssueNumber()), true);
    addCheckBox("Articolo scientifico", "isScientificPaper", article.getIsScientificPaper());
    addImageSelector("Copertina", "cover", article.getCoverImagePath());
}
